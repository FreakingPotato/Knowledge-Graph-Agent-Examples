#!/usr/bin/env python3
"""Agent R - promote diffusion_models evidence candidates.

Reads:
  - diffusion_models/metadata/ontology.json
  - diffusion_models/metadata/scored_gfm.json
  - diffusion_models/metadata/scored_multimodal.json
  - diffusion_models/metadata/scored_applications.json
  - diffusion_models/metadata/papers.json
  - diffusion_models/metadata/candidates.json

Writes:
  - diffusion_models/metadata/evidence.json (curated, mirrors ontology)
  - diffusion_models/metadata/candidates.json (unified, scored, tiered)
  - diffusion_models/metadata/promotion_log.md (human-readable trace)
...
def build_evidence_doc(
    ...
    return {
        "schema_version": "0.1",
        "topic_id": ontology.get("topic_id", "diffusion_models"),
...
def build_log(
    ...
    lines.append(
        ...
        "via the diffusion_models paper registry rather than a primary WCM list."
    )
    lines.append("")
    return "\n".join(lines)
