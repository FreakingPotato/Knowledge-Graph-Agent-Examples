#!/usr/bin/env python3
"""Generate multi-sentence evidence screenshots for diffusion_models_evidence.json.
...
Schema written into ``diffusion_models/metadata/evidence.json`` (per evidence
point):
...
LOG_FILE = ROOT / "diffusion_models" / "metadata" / "screenshot_log.json"
PDF_DIR = ROOT / "pdfs"
ASSETS_DIR = ROOT / "diffusion_models" / "assets" / "evidence"
...
def _paper_registry() -> dict[str, dict]:
    ...
    out: dict[str, dict] = {}
    for path, key in (
        (ROOT / "diffusion_models" / "metadata" / "papers.json", "papers"),
    ):
        if not path.is_file():
            continue
        try:
...
def _process_one(ep: dict, force: bool, log: dict) -> str:
    ...
    try:
        ...
        if not top_scored:
            ...
            href = f"diffusion_models/assets/evidence/{eid}__01.png"
        ...
        for i, g in enumerate(groups, start=1):
            ...
            try:
                ...
                continue
            href = f"diffusion_models/assets/evidence/{eid}__{i:02d}.png"
            (ROOT / href).write_bytes(png_bytes)
