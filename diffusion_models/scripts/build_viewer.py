#!/usr/bin/env python3
"""Build a self-contained ``diffusion_models/index.html`` viewer.
...
The HTML mirrors the assets/evidence/*.png paths into diffusion_models/
so the viewer is portable: serve the diffusion_models/ directory and
nothing breaks.

Usage::

    python diffusion_models/scripts/build_viewer.py
    python diffusion_models/scripts/build_viewer.py --out custom.html
"""
...
TOPIC_DIR = ROOT / "diffusion_models"
...
def _embedded_evidence(ev: dict, out_dir: Path) -> dict:
    ...
    viewer file (which sits at diffusion_models/index.html), and ensure
    the PNG files actually live next to the viewer.

    Source paths in evidence.json look like
    ``diffusion_models/assets/evidence/<file>.png``; the viewer needs
...
def _ensure_assets(papers_dir_evidence: Path, out_dir: Path) -> None:
    """No copy required: viewer at diffusion_models/index.html shares
    the same parent directory as diffusion_models/assets/evidence/.
...
function mount() {
  ...
  if (!EVIDENCE || !Array.isArray(EVIDENCE.paradigms)) {
    root.innerHTML = `<div class="thesis">No evidence yet — populate <code>diffusion_models/metadata/evidence.json</code> and rebuild.</div>`;
    return;
  }
...
function downloadTSV() {
  ...
  a.href = url; a.download = `diffusion_models_evidence_${ts}.tsv`;
