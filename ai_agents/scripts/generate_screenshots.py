#!/usr/bin/env python3
"""Generate multi-sentence evidence screenshots for ai_agents_evidence.json.
...
Schema written into ``ai_agents/metadata/evidence.json`` (per evidence
point):
...
LOG_FILE = ROOT / "ai_agents" / "metadata" / "screenshot_log.json"
PDF_DIR = ROOT / "pdfs"
...
def _paper_registry() -> dict[str, dict]:
    ...
    out: dict[str, dict] = {}
    for path, key in (
        (ROOT / "ai_agents" / "metadata" / "papers.json", "papers"),
    ):
        if not path.is_file():
            continue
        try:
...
def _process_one(ep: dict, force: bool, log: dict) -> str:
    ...
    try:
        ...
        if not top_scored:
            ...
            href = f"ai_agents/assets/evidence/{eid}__01.png"
        ...
        for i, g in enumerate(groups, start=1):
            ...
            try:
                ...
                continue
            href = f"ai_agents/assets/evidence/{eid}__{i:02d}.png"
            (ROOT / href).write_bytes(png_bytes)
