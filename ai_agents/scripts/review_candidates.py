...
multi-claim cap), and:
  ...
  1. Appends new MMG evidence points to the right claim in
     ``ai_agents/metadata/evidence.json`` (existing rows untouched).
  2. Overwrites ``ai_agents/metadata/candidates.json`` with the
     unified candidates record (every (paper, claim) row carries
     ``tier`` and ``promotion_status``).
  3. Writes a human-readable trace to
     ``ai_agents/metadata/promotion_log.md``.
...
EVIDENCE_FILE = META / "ai_agents_evidence.json"
