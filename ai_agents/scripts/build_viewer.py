#!/usr/bin/env python3
"""Build a self-contained ``ai_agents/index.html`` viewer.
...
The HTML mirrors the assets/evidence/*.png paths into ai_agents/
so the viewer is portable: serve the ai_agents/ directory and
nothing breaks.

Usage::

    python ai_agents/scripts/build_viewer.py
    python ai_agents/scripts/build_viewer.py --out custom.html
"""
...
def _embedded_evidence(ev: dict, out_dir: Path) -> dict:
    ...
    viewer file (which sits at ai_agents/index.html), and ensure
    the PNG files actually live next to the viewer.

    Source paths in evidence.json look like
    ``ai_agents/assets/evidence/<file>.png``; the viewer needs
...
def _ensure_assets(papers_dir_evidence: Path, out_dir: Path) -> None:
    """No copy required: viewer at ai_agents/index.html shares
    the same parent directory as ai_agents/assets/evidence/.
...
function mount() {
  ...
  if (!EVIDENCE || !Array.isArray(EVIDENCE.paradigms)) {
    root.innerHTML = `<div class="thesis">No evidence yet — populate <code>ai_agents/metadata/evidence.json</code> and rebuild.</div>`;
    return;
  }
...
function downloadTSV() {
  ...
  a.href = url; a.download = `ai_agents_evidence_${ts}.tsv`;
